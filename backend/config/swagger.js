...
const options = {
  definition: {
      ...
      schemas: {
        ...
        Notification: {
          ...
          properties: {
            ...
            type: { type: 'string', enum: ['TASK_ASSIGNED', 'TASK_STATUS_CHANGED', 'COMMENT_ADDED', 'COMMENT_MENTIONED', 'GOAL_ASSIGNED', 'FEEDBACK_RECEIVED'] },
            message: { type: 'string' },
            relatedType: { type: 'string', enum: ['Task', 'Comment', 'Goal', 'Feedback'], nullable: true },
        ...
        UserFeedback: {
          ...
          properties: {
            _id: { type: 'string' },
            targetUser: { $ref: '#/components/schemas/UserRef' },
            givenBy: { $ref: '#/components/schemas/UserRef' },
            project: { type: 'object', properties: { _id: { type: 'string' }, name: { type: 'string' } } },
            rating: { type: 'integer', minimum: 1, maximum: 5, example: 4 },
            comment: { type: 'string', example: 'Great work on the sprint deliverables' },
            completedInTimeline: { type: 'boolean', nullable: true },
            companyId: { type: 'string' },
            createdAt: { type: 'string', format: 'date-time' },
            updatedAt: { type: 'string', format: 'date-time' },
          },
        ...
        UserGoal: {
          ...
          properties: {
            ...
            project: { type: 'object', properties: { _id: { type: 'string' }, name: { type: 'string' }, status: { type: 'string' } } },
            month: { type: 'string', format: 'date-time', description: 'First day of the goal month' },
            targetCount: { type: 'integer', minimum: 1, example: 10 },
            description: { type: 'string', example: 'Complete all sprint tasks' },
            companyId: { type: 'string' },
            createdAt: { type: 'string', format: 'date-time' },
            updatedAt: { type: 'string', format: 'date-time' },
          },
    ...
    tags: [
      ...
      { name: 'Profile', description: 'User profile and analytics' },
      { name: 'Feedback', description: 'Manager-to-member performance feedback' },
      { name: 'Goals', description: 'Monthly task goals per user and project' },
    ],
  },
  apis: ['./routes/*.js'],
};
